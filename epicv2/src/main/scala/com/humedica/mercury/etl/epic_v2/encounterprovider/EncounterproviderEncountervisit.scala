...
/**
  ...
  */
class EncounterproviderEncountervisit(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {


  tables = List("temptable:epic_v2.clinicalencounter.ClinicalencounterTemptable",
    "pat_enc_hsp", "inptbilling_acct","zh_claritydept","hh_pat_enc","hsp_acct_pat_csn","cdr.map_predicate_values")
  ...
  columnSelect = Map(
    "temptable" -> List("ARRIVALTIME", "DISCH_DISP_C", "PAT_ID", "HOSP_ADMSN_TIME", "HOSP_DISCH_TIME", "PAT_ENC_CSN_ID", "ATTND_PROV_ID", "UPDATE_DATE", "ENC_TYPE_C","HSP_ACCOUNT_ID",
      "ADT_PAT_CLASS_C", "ADMIT_SOURCE_C","APPT_STATUS_C", "DEPARTMENT_ID", "EFFECTIVE_DEPT_ID", "ENCOUNTER_PRIMARY_LOCATION", "SERV_AREA_ID"
      ,"ADMISSION_PROV_ID", "ATTND_PROV_ID", "DISCHARGE_PROV_ID","VISIT_PROV_ID", "REFERRAL_SOURCE_ID"),
  ...
  beforeJoin = Map(
    "temptable" -> ((df: DataFrame) => {
      ...
      val df1 = fil1.repartition(1000)
      val fpiv = unpivot(
        Seq("ADMISSION_PROV_ID","ATTND_PROV_ID","DISCHARGE_PROV_ID","VISIT_PROV_ID","REFERRAL_SOURCE_ID"),
        Seq("Admitting Provider","Attending Provider","Discharge Provider","Visit Provider","Referring Provider"), typeColumnName = "PROVIDER_ROLE")
      fpiv("PROVIDER_ID",df1)
    }),
    "pat_enc_hsp" -> ((df: DataFrame) => {
      val groups = Window.partitionBy(df("PAT_ENC_CSN_ID"),df("HSP_ACCOUNT_ID")).orderBy(df("UPDATE_DATE").desc)
      val df1 = df.withColumn("peh_rn", row_number.over(groups))
        .filter("peh_rn = 1")
        .withColumnRenamed("DEPARTMENT_ID", "DEPARTMENT_ID_PEH")
        .withColumnRenamed("HSP_ACCOUNT_ID", "HSP_ACCOUNT_ID_PEH")
        .withColumnRenamed("PAT_ENC_CSN_ID", "PAT_ENC_CSN_ID_PEH")
        .withColumnRenamed("UPDATE_DATE", "UPDATE_DATE_PEH")
      df1.repartition(1000)
    }),
  ...
  join = (dfs: Map[String, DataFrame]) => {
    dfs("temptable")
      .join(dfs("pat_enc_hsp"), dfs("pat_enc_hsp")("PAT_ENC_CSN_ID_PEH") === dfs("temptable")("PAT_ENC_CSN_ID") && dfs("pat_enc_hsp")("HSP_ACCOUNT_ID_PEH") === dfs("temptable")("HSP_ACCOUNT_ID"), "left_outer")
  ...
  map = Map(
    ...
    "PATIENTID" -> mapFrom("PAT_ID_EV", nullIf = Seq("-1")),
    ...
    "PROVIDERID" -> ((col: String, df: DataFrame) => {
      df.withColumn(col, when(df("PROVIDER_ROLE") === lit("Referring Provider"), concat_ws("", lit("ref."),df("PROVIDER_ID")))
        .otherwise(df("PROVIDER_ID")))
    }),
    "PROVIDERROLE" -> mapFrom("PROVIDER_ROLE")
  )

  afterMap = (df: DataFrame) => {
    val groups = Window.partitionBy(df("ENCOUNTERID"),df("PROVIDERROLE"),when(df("PROVIDERROLE") === lit("Attending Provider"), df("PROVIDERID"))).orderBy(df("UPDATE_DATE").desc,df("FILEID").desc, df("FILEID_ia").desc)
    val addColumn = df.withColumn("rn", row_number.over(groups))
    addColumn.filter("rn = 1 AND PROVIDERID is not null AND PROVIDERID <> '-1'").drop("rn")
  }
