...
class MedmapsourceAlert (config: Map[String, String]) extends EntitySource(config: Map[String, String]){

  tables = List("tempallergies:epic_v2.allergies.AllergiesAlert")
...
// TEST
// val a = new MedmapsourceAlert(cfg); val al = build(a) ; al.orderBy(desc("NUM_RECS")).show(200)
