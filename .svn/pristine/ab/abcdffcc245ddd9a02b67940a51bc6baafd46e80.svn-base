...
/**
  ...
  */
class OrxpharmacyclaimOrxpharmacyclaimPreAccID(config: Map[String,String]) extends EntitySource(config: Map[String,String]) {

    tables = List("claim", "ndc.ndc", "pharmacy.pharmacy", "history.member_eligibility_history","carriersblc2v2.carriers_blc2_v2")
    ...
    columnSelect = Map(
        "history.member_eligibility_history" -> List("FLCPDA", "FLCQDA","FLAACD", "FLACCD", "FLADCD", "FLABCD"),
        "claim" -> List("CLIENTFLAG", "AWPUNITCST", "CTYPEUCOST", "RSPCOPAY", "CLTATRDED", "CLTDISPFEE", "CLAIMSTS",
            ...
            "DECIMALQTY", "CLMSEQNBR", "CARRIERID", "GROUPID", "MEMBERID", "DAYSSUPPLY",  "RXNUMBER"),
        "ndc.ndc" -> List("CODE", "AHFS_THERAPEUTIC_CLASS", "BRAND_NAME", "DRUG_PRICE_TYPE_CODE", "DRUG_STRENGTH_DESC", "GENERIC", "SPECIFIC_THERAPEUTIC_CLASS", "EFF_DATE", "END_DATE"),
    ...
    join = (dfs: Map[String, DataFrame]) => {
        var claim = dfs("claim")
        var ndc = dfs("ndc.ndc")
        var member_eligibility_history = dfs("history.member_eligibility_history")
        var carriers_blc2_v2 = dfs("carriersblc2v2.carriers_blc2_v2")
        carriers_blc2_v2=carriers_blc2_v2.filter("BUSINESS_LINE_CODE == 'COM'")
        ...
        claim.join(carriers_blc2_v2,
            ...
            (carriers_blc2_v2("OPTYPE")==="1" && claim("CARRIERID").contains(carriers_blc2_v2("CARRIER_ID")))), "inner")
                .join(member_eligibility_history, member_eligibility_history("FLAACD") === claim("CARRIERID") &&
                        member_eligibility_history("FLADCD") === claim("GRPID") &&
                        member_eligibility_history("FLABCD") === claim("MEMBERID") &&
                        unix_timestamp(trim(claim("DTEFILLED")), "yyyyMMdd").between(
                    parseORXCrossixDateObj("FLCPDA", member_eligibility_history),
                    parseORXCrossixDateObj("FLCQDA", member_eligibility_history))
                    , "left_outer")
    ...
    map = Map(
        ...
        "ORX_INDIVIDUAL_ID" -> ((col: String, df: DataFrame) => df.withColumn(col, sha2(concat(df("CARRIERID"), df("FLACCD"), df("GRPID"), df("MEMBERID")), 256))),
...
//   build(new OrxpharmacyclaimOrxpharmacyclaimPreAccID(cfg), allColumns=true).distinct.write.parquet(cfg.get("EMR_DATA_ROOT").get+"/ORX_PHARMACY_CLAIM")
