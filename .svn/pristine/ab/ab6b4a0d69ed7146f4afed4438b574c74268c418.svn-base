...
/**
  * Created by rbabu on 11/17/17.
  */
object CrossixRxorderOpUtility {
    ...
    def readTable(sqlContext:SQLContext, dateStr:String, table1:String, cfg: Map[String, String]): DataFrame = {
            ...
            var table = table1 + "/STAGING/*"
            //var path = cfg.get("EMR_DATA_ROOT").get + "/" + table
            var path = home_dir + "/data/rxorder/weekly/"+dateStr+"/"+table
    ...
    def processEPTempPatientDetails(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String]): Unit = {
        ...
        PATREG = PATREG.withColumn("FILE_ID", PATREG("FILEID").cast("String"))
        var shelf_manifest_entry = readDictionaryTable(sqlContext, "SHELF_MANIFEST_ENTRY_WCLSID",cfg).withColumnRenamed("CLIENT_DATA_SRC_ID", "CLIENT_DS_ID_sme").withColumnRenamed("GROUPID", "GROUPID_sme")
        PATREG = PATREG.join(shelf_manifest_entry, Seq("FILE_ID"), "inner").withColumnRenamed("GROUPID_sme", "GROUPID_pat").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_pat")

        var PA = PATREG.withColumnRenamed("UPDATE_DATE", "PA_UPDATE_DATE").select("FILE_ID", "GROUPID_pat","CLIENT_DS_ID_pat", "PAT_ID", "PAT_MRN_ID", "BIRTH_DATE", "DEATH_DATE", "PAT_STATUS_C",
        ...
        IDENTITY_ID = IDENTITY_ID.join(shelf_manifest_entry, Seq("FILE_ID"), "inner").withColumnRenamed("GROUPID_sme", "GROUPID_id1").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_id1")
        ...
        var PAT_RACE = readTable(sqlContext, dateStr, "PAT_RACE", cfg).withColumnRenamed("UPDATE_DATE","RA_UPDATE_DATE").select("FILE_ID", "PATIENT_RACE_C", "PAT_ID", "RA_UPDATE_DATE")
        PAT_RACE = PAT_RACE.join(shelf_manifest_entry, Seq("FILE_ID"), "inner").withColumnRenamed("GROUPID_sme", "GROUPID_pc").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_pc")
        ...
        var PATIENT_3 = readTable(sqlContext, dateStr, "PATIENT_3", cfg).select("FILE_ID",   "IS_TEST_PAT_YN", "PAT_ID", "UPDATE_DATE")
        PATIENT_3 = PATIENT_3.join(shelf_manifest_entry, Seq("FILE_ID"), "inner").withColumnRenamed("GROUPID_sme", "GROUPID_patient").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_patient")
        ...
        var PATIENT_FYI_FLAGS = readTable(sqlContext, dateStr, "PATIENT_FYI_FLAGS", cfg).select("FILE_ID",  "PAT_FLAG_TYPE_C","PATIENT_ID")
        PATIENT_FYI_FLAGS = PATIENT_FYI_FLAGS.join(shelf_manifest_entry, Seq("FILE_ID"), "inner").withColumnRenamed("GROUPID_sme", "GROUPID_pyf").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_pyf")

        var ZH_STATE = readDictionaryTable(sqlContext, "ZH_STATE", cfg).withColumnRenamed("NAME", "ZH_STATE_NAME")
        ZH_STATE = ZH_STATE.withColumnRenamed("FILEID", "FILE_ID").join(shelf_manifest_entry, Seq("FILE_ID"), "inner").withColumnRenamed("GROUPID_sme", "GROUPID_zs").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_zs")
    ...
    def clinicalEncounterCernverV2(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String]) = {
        ...
        var ENCOUNTER = readTable(sqlContext, dateStr, "ENCOUNTER",cfg).select( "FILE_ID", "ACTIVE_IND", "CONTRIBUTOR_SYSTEM_CD", "ENCNTR_ID", "DISCH_DT_TM", "INPATIENT_ADMIT_DT_TM"
        ...
        ENCOUNTER = ENCOUNTER.join(shelf_manifest_entry, ENCOUNTER("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_enc").withColumnRenamed("GROUPID_sme", "GROUPID_enc")
        ...
        var HUM_DRG = readTable(sqlContext, dateStr, "HUM_DRG",cfg).select("FILE_ID", "NOMENCLATURE_ID", "RISK_OF_MORTALITY_CD", "SEVERITY_OF_ILLNESS_CD", "DRG_PRIORITY", "UPDT_DT_TM", "ACTIVE_IND", "ENCNTR_ID", "PERSON_ID")
        var HUM_DRG_1 = HUM_DRG.filter(HUM_DRG("ACTIVE_IND") === "1").withColumnRenamed("NOMENCLATURE_ID", "HD_NOMENCLATURE_ID")
        HUM_DRG_1 = HUM_DRG_1.join(shelf_manifest_entry, HUM_DRG_1("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_hd").withColumnRenamed("GROUPID_sme", "GROUPID_hd")

        var ZH_NOMENCLATURE = readDictionaryTable(sqlContext, "ZH_NOMENCLATURE",cfg).select("FILEID", "SOURCE_VOCABULARY_CD", "NOMENCLATURE_ID", "SOURCE_IDENTIFIER")
        var ZH_NOMENCLATURE_FILTER = ZH_NOMENCLATURE.filter(ZH_NOMENCLATURE("SOURCE_IDENTIFIER").isNotNull)
        ZH_NOMENCLATURE_FILTER = ZH_NOMENCLATURE_FILTER.join(shelf_manifest_entry, ZH_NOMENCLATURE_FILTER("FILEID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_zmc").withColumnRenamed("GROUPID_sme", "GROUPID_zmc")
        ...
        RCV = RCV.join(shelf_manifest_entry, RCV("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_rcv").withColumnRenamed("GROUPID_sme", "GROUPID_rcv")
        ...
        SCV = SCV.join(shelf_manifest_entry, SCV("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_scv").withColumnRenamed("GROUPID_sme", "GROUPID_scv")
        ...
        var ENC_ALIAS = readTable(sqlContext, dateStr,  "ENC_ALIAS",cfg).select("FILE_ID","ENCNTR_ALIAS_TYPE_CD", "ENCNTR_ID", "ALIAS", "UPDT_DT_TM")
        ENC_ALIAS = ENC_ALIAS.join(shelf_manifest_entry, ENC_ALIAS("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_ea").withColumnRenamed("GROUPID_sme", "GROUPID_ea")
    ...
    def rxorder_cernerv2(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String]) = {
        ...
        var O_MS = readTable(sqlContext, dateStr, "ORDERS",cfg).select("FILE_ID", "ORDER_DETAIL_DISPLAY_LINE", "ENCNTR_ID", "PERSON_ID", "ACTIVE_IND", "CATALOG_CD", "ACTIVITY_TYPE_CD", "ORDER_ID", "UPDT_DT_TM", "ORIG_ORD_AS_FLAG", "ORIG_ORDER_DT_TM")
        var shelf_manifest_entry = readDictionaryTable(sqlContext, "SHELF_MANIFEST_ENTRY_WCLSID",cfg).withColumnRenamed("CLIENT_DATA_SRC_ID", "CLIENT_DS_ID_sme").withColumnRenamed("GROUPID", "GROUPID_sme")
        O_MS = O_MS.join(shelf_manifest_entry, O_MS("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_oms").withColumnRenamed("GROUPID_sme", "GROUPID_oms")

        var ZH_CATALOG_CD_MS =readDictionaryTable(sqlContext, "ZH_CODE_VALUE",cfg).withColumnRenamed("DISPLAY", "DISPLAY_zh")
        ZH_CATALOG_CD_MS=ZH_CATALOG_CD_MS.withColumn("FILE_ID",ZH_CATALOG_CD_MS("FILEID").cast("String")).select("FILE_ID", "CODE_VALUE", "CODE_SET", "DISPLAY_zh")
        ZH_CATALOG_CD_MS = ZH_CATALOG_CD_MS.join(shelf_manifest_entry, ZH_CATALOG_CD_MS("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_zms").withColumnRenamed("GROUPID_sme", "GROUPID_zms")
        ...
        OA=OA.withColumnRenamed("ORDER_ID", "OA_ORDER_ID").select("FILE_ID",   "ACTION_TYPE_CD", "OA_ORDER_ID", "ACTION_SEQUENCE", "UPDT_DT_TM", "ORDER_DT_TM", "ORDER_PROVIDER_ID", "ORDER_STATUS_CD", "STOP_TYPE_CD")
        OA = OA.join(shelf_manifest_entry, OA("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_oa").withColumnRenamed("GROUPID_sme", "GROUPID_oa")
        ...
        OD = OD.join(shelf_manifest_entry, OD("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_od").withColumnRenamed("GROUPID_sme", "GROUPID_od")

        var ZH_FIELD_VALUE = readDictionaryTable(sqlContext, "ZH_CODE_VALUE",cfg)
        ZH_FIELD_VALUE=ZH_FIELD_VALUE.withColumn("FILE_ID", ZH_FIELD_VALUE("FILEID").cast("String"))      .select("FILE_ID",  "CODE_VALUE", "DESCRIPTION")
        ZH_FIELD_VALUE = ZH_FIELD_VALUE.join(shelf_manifest_entry, ZH_FIELD_VALUE("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_zfv").withColumnRenamed("GROUPID_sme", "GROUPID_zfv")
        ...
        ZHOCIR = ZHOCIR.join(shelf_manifest_entry, ZHOCIR("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_zc").withColumnRenamed("GROUPID_sme", "GROUPID_zc")
        ...
        MID = MID.join(shelf_manifest_entry, MID("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_mid").withColumnRenamed("GROUPID_sme", "GROUPID_mid")
...
/*
    def processEP2RxOrder(sqlContext: SQLContext, dateStr:String, entitySrc3:EntitySource, cfg: Map[String,String])= {
        //writeGenerated(Toolkit.build(entitySrc1, allColumns=true), "CLINICALENCOUNTERTEMPTABLE", cfg)
        //writeGenerated(Toolkit.build(entitySrc2, allColumns=true), "CLINICALENCOUNTERVISIT", cfg)
        writeGenerated(Toolkit.build(entitySrc3, allColumns=true).withColumnRenamed("GRPID1", "GROUPID").withColumnRenamed("CLSID1", "CLIENT_DS_ID"), dateStr, "EPRXORDER", cfg)
    }
*/
    def mergeAllRxOrders(sqlContext:SQLContext, dateStr:String, cfg: Map[String,String]) = {

        println("Writing ASRXORDER...")
        var s = readGenerated(sqlContext, dateStr,"ASRXORDER", cfg)

        s = s.withColumn("FACILITYID", s("FACILITYID").cast("String"))
                .withColumn("RXID", s("RXID").cast("String"))
                .withColumn("ENCOUNTERID", s("ENCOUNTERID").cast("String"))
                .withColumn("LOCALMEDCODE", s("LOCALMEDCODE").cast("String"))
                .withColumn("LOCALPROVIDERID", s("LOCALPROVIDERID").cast("String"))
                .withColumn("LOCALROUTE", s("LOCALROUTE").cast("String"))
                .withColumn("FILLNUM", s("FILLNUM").cast("String"))
                .withColumn("ORDERSTATUS", s("ORDERSTATUS").cast("String"))
                .withColumn("LOCALDAYSUPPLIED", s("LOCALDAYSUPPLIED").cast("String"))
                .withColumn("LOCALTOTALDOSE", s("LOCALTOTALDOSE").cast("String"))
                .withColumn("LOCALDOSEFREQ", s("LOCALDOSEFREQ").cast("String"))
                .withColumn("LOCALDURATION", s("LOCALDURATION").cast("String"))
                .withColumn("LOCALGPI", s("LOCALGPI").cast("String"))
                .select("GROUPID", "DATASRC", "FACILITYID", "RXID",
                    "ENCOUNTERID", "PATIENTID", "LOCALMEDCODE", "LOCALDESCRIPTION", "LOCALPROVIDERID",
                    "ISSUEDATE", "DISCONTINUEDATE", "LOCALSTRENGTHPERDOSEUNIT", "LOCALSTRENGTHUNIT", "LOCALROUTE",
                    "EXPIREDATE", "QUANTITYPERFILL", "FILLNUM", "SIGNATURE", "ORDERTYPE",
                    "ORDERSTATUS", "ALTMEDCODE", "MAPPEDNDC", "MAPPEDGPI", "MAPPEDNDC_CONF",
                    ...
                    "ALLOWEDAMOUNT", "PAIDAMOUNT", "CHARGE", "DCC", "RXNORM_CODE",
                    "ACTIVE_MED_FLAG", "ROW_SOURCE", "MODIFIED_DATE")
        appendGenerated(s,dateStr, "CONSOLIDATED_RXORDER", cfg)

        println("Writing EPRXORDER...")
        var s1 = readGenerated(sqlContext, dateStr,"EPRXORDER", cfg)
        s1 = s1.select("GROUPID", "DATASRC", "FACILITYID", "RXID",
            "ENCOUNTERID", "PATIENTID", "LOCALMEDCODE", "LOCALDESCRIPTION", "LOCALPROVIDERID",
            "ISSUEDATE", "DISCONTINUEDATE", "LOCALSTRENGTHPERDOSEUNIT", "LOCALSTRENGTHUNIT", "LOCALROUTE",
            "EXPIREDATE", "QUANTITYPERFILL", "FILLNUM", "SIGNATURE", "ORDERTYPE",
            "ORDERSTATUS", "ALTMEDCODE", "MAPPEDNDC", "MAPPEDGPI", "MAPPEDNDC_CONF",
        ...
        appendGenerated(s1,dateStr, "CONSOLIDATED_RXORDER", cfg)




        println("Writing CRRXORDER...")
        var s2 = readGenerated(sqlContext,dateStr, "CR2RXORDER", cfg)
        s2=s2.withColumn("VENUE", s2("VENUE").cast("String")).withColumn("LOCALTOTALDOSE", s2("LOCALTOTALDOSE").cast("String"))
                .select("GROUPID", "DATASRC", "FACILITYID", "RXID",
                    "ENCOUNTERID", "PATIENTID", "LOCALMEDCODE", "LOCALDESCRIPTION", "LOCALPROVIDERID",
                    "ISSUEDATE", "DISCONTINUEDATE", "LOCALSTRENGTHPERDOSEUNIT", "LOCALSTRENGTHUNIT", "LOCALROUTE",
                    "EXPIREDATE", "QUANTITYPERFILL", "FILLNUM", "SIGNATURE", "ORDERTYPE",
        ...
        appendGenerated(s2,dateStr, "CONSOLIDATED_RXORDER", cfg)
    ...
    def processASTempPatientDetails(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String]) = {
        ...
        var shelfManifestEntry = readDictionaryTable(sqlContext, "SHELF_MANIFEST_ENTRY_WCLSID", cfg).withColumnRenamed("CLIENT_DATA_SRC_ID", "CLIENT_DS_ID_sme").withColumnRenamed("GROUPID", "GROUPID_sme")
        PATDEM = PATDEM.join(shelfManifestEntry, Seq("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_pat").withColumnRenamed("GROUPID_sme", "GROUPID_pat")
    ...
    def processCRTempPatientDetails(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String]) = {
        ...
        var person = readTable(sqlContext,dateStr, "PERSON",cfg).select("FILE_ID", "NAME_LAST", "NAME_FULL_FORMATTED","PERSON_ID", "BIRTH_DT_TM", "DECEASED_DT_TM", "NAME_FIRST", "UPDT_DT_TM", "religion_cd", "BEG_EFFECTIVE_DT_TM",
        ...
        person = person.join(shelf_manifest_entry, person("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_person").withColumnRenamed("GROUPID_sme", "GROUPID_person")
        ...
        var personAlias = readTable(sqlContext,dateStr, "PERSON_ALIAS",cfg).select("FILE_ID", "person_alias_type_cd", "alias_pool_cd", "person_id", "end_effective_dt_tm","alias", "updt_dt_tm", "active_ind").withColumnRenamed("person_id", "person_alias_id")
        personAlias = personAlias.join(shelf_manifest_entry, personAlias("FILE_ID")===shelf_manifest_entry("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_pa").withColumnRenamed("GROUPID_sme", "GROUPID_pa")
        ...
        var address = readDictionaryTable(sqlContext, "ADDRESS_V1", cfg).select("FILEID", "ZIPCODE", "PARENT_ENTITY_ID")
        ...
        address = address.join(shelf_manifest_entry, Seq("FILE_ID"), "inner").withColumnRenamed("CLIENT_DS_ID_sme", "CLIENT_DS_ID_address").withColumnRenamed("GROUPID_sme", "GROUPID_address")
        ...
        def processCRTempPatientDetails1(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String]) = {
        var shelf_manifest_entry = readDictionaryTable(sqlContext, "SHELF_MANIFEST_ENTRY_WCLSID",cfg).withColumnRenamed("CLIENT_DATA_SRC_ID", "CLIENT_DS_ID_sme").withColumnRenamed("GROUPID", "GROUPID_sme")
...
/*
       // CONTAINS CHANGE FOR JIRA DFLS-2
    ...
    def rxorder_patient_phi(sqlContext: SQLContext,dateStr:String, cfg: Map[String,String]) = {
        ...
        pat_sum_join = pat_sum_join.withColumn("SSN", when(pat_sum_join("SSN").isNull, "NULL")
                .when(length(regexp_extract(pat_sum_join("SSN"), "[0-9]{9}", 0)) !== 9, "LEN")
                    ...
                    .when(pat_sum_join("SSN").isin("000000000", "111111111", "222222222", "333333333", "444444444", "555555555", "666666666", "777777777", "888888888", "999999999"), "SAME")
        ...
        var FINAL_PATIENT_SUMMARY = pat_sum_join.select("PATIENTID", "CDR_GRP_MPI", "DCDR_GRP_MPI", "GROUPID", "DOB", "FIRST_NAME",
                    "LAST_NAME", "MAPPED_GENDER", "MAPPED_ETHNICITY", "MAPPED_RACE", "MAPPED_ZIPCODE", "SSN")

        writeGenerated(FINAL_PATIENT_SUMMARY,dateStr, "RXORDER_PHI_TEST", cfg)
    }
    ...
    */
    ...
    def rxorder_patient_phi(sqlContext: SQLContext,dateStr:String, cfg: Map[String,String]) = {
        ...
        var FINAL_PATIENT_SUMMARY = JOIN_PATIENT_SUMMARY.select("PATIENTID", "CDR_GRP_MPI", "DCDR_GRP_MPI", "GROUPID", "DOB", "FIRST_NAME",
            "LAST_NAME", "MAPPED_GENDER", "MAPPED_ETHNICITY", "MAPPED_RACE", "MAPPED_ZIPCODE")
    ...
    def processASRxOrder(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String], entirySrc:EntitySource) = {
        var s = CrossixGenericUtil.build(new RxordersandprescriptionsErx(cfg)).withColumnRenamed("GRPID1", "GROUPID").withColumnRenamed("CLSID", "CLIENT_DS_ID")
        CrossixRxorderOpUtility.writeGenerated(s.distinct(), dateStr, "ASRXORDER", cfg)
    }

    def processEP2RxOrder(sqlContext: SQLContext, dateStr:String, entitySrc3:EntitySource, cfg: Map[String,String])= {
        CrossixRxorderOpUtility.writeGenerated(CrossixGenericUtil.build(new RxordersandprescriptionsMedorders(cfg)).withColumnRenamed("GRPID1", "GROUPID").withColumnRenamed("CLSID1", "CLIENT_DS_ID"), dateStr, "EPRXORDER", cfg)
    }
    ...
    def postValidateRXORDERTableColumns(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String]): Unit = {
        var home_dir = cfg.get("HOME_DIR").get

        println("Validating ... "+home_dir+"/data/rxorder/weekly/"+dateStr+"/GENERATED/RXORDER")
        var RX_ORDER = sqlContext.read.parquet(home_dir+"/data/rxorder/weekly/"+dateStr+"/GENERATED/RXORDER")
        var result = arrayEquals(Array("PATIENTID", "CDR_GRP_MPI", "DCDR_GRP_MPI", "ORDERVSPRESCRIPTION", "ENCOUNTERID", "GROUPID",
            "FACILITYID", "LOCALMEDCODE", "LOCALPROVIDERID", "ISSUEDATE", "ALTMEDCODE", "ORDERTYPE", "ORDERSTATUS", "QUANTITYPERFILL",
            "FILLNUM", "RXID", "LOCALSTRENGTHPERDOSEUNIT", "LOCALSTRENGTHUNIT", "LOCALROUTE", "MAPPEDNDC", "LOCALDESCRIPTION_PHI", "DATASRC",
            "VENUE", "MAPPEDNDC_CONF", "MAPPEDGPI", "MAPPEDGPI_CONF", "HUM_MED_KEY", "MAP_USED", "LOCALDAYSUPPLIED",
            "LOCALFORM", "LOCALQTYOFDOSEUNIT", "LOCALTOTALDOSE", "LOCALDOSEFREQ", "LOCALINFUSIONVOLUME", "LOCALINFUSIONDURATION", "LOCALNDC",
            "LOCALGPI", "LOCALDOSEUNIT", "LOCALDISCHARGEMEDFLG", "LOCALDAW", "HUM_GEN_MED_KEY", "MSTRPROVID", "HTS_GENERIC", "HTS_GENERIC_EXT",
            "CLIENT_DS_ID", "DISCONTINUEDATE", "EXPIREDATE", "DISCONTINUEREASON", "DCC", "NDC11","ROUTE_CUI", "DOSEFREQ_CUI",
            "MAPPED_ORDERSTATUS", "RXNORM_CODE", "PREFERREDNDC", "PREFERREDNDC_SOURCE", "ORIGIN"), RX_ORDER.columns)
        if(result == false) {
            throw new Exception("RXORDER ==> FAILED COLUMN COMPARISON")
        }
        println(  dateStr+ " : RXORDER ROWCOUNT :"+RX_ORDER.count())
        CrossixGenericUtil.summary(RX_ORDER)
    }


    def postValidateRXORDERPHITableColumns(sqlContext: SQLContext, dateStr:String, cfg: Map[String,String]): Unit = {
        var home_dir = cfg.get("HOME_DIR").get

        println("Validating ... "+home_dir+"/data/rxorder/weekly/"+dateStr+"/GENERATED/RXORDER_PHI")
        var RX_ORDER_PHI = sqlContext.read.parquet(home_dir+"/data/rxorder/weekly/"+dateStr+"/GENERATED/RXORDER_PHI")
        var result = arrayEquals(Array("PATIENTID", "CDR_GRP_MPI", "DCDR_GRP_MPI", "GROUPID", "DOB", "FIRST_NAME",
            "LAST_NAME", "MAPPED_GENDER", "MAPPED_ETHNICITY", "MAPPED_RACE", "MAPPED_ZIPCODE"), RX_ORDER_PHI.columns)
        if(result == false) {
            throw new Exception("RXORDER_PHI ==> FAILED COLUMN COMPARISON")
        }
        println(  dateStr+ " : RXORDER_PHI ROWCOUNT :"+RX_ORDER_PHI.count())
        CrossixGenericUtil.summary(RX_ORDER_PHI)
    }
