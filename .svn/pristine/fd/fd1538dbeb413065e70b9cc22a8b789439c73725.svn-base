...
class EncounterproviderDischargeprovider(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  columnSelect = Map(
    "temptable" -> List("ARRIVALTIME", "DISCH_DISP_C", "PAT_ID", "HOSP_ADMSN_TIME", "HOSP_DISCH_TIME", "PAT_ENC_CSN_ID", "ATTND_PROV_ID", "UPDATE_DATE", "ENC_TYPE_C", "HSP_ACCOUNT_ID",
      "ADT_PAT_CLASS_C", "ADMIT_SOURCE_C", "APPT_STATUS_C", "DEPARTMENT_ID", "EFFECTIVE_DEPT_ID", "ENCOUNTER_PRIMARY_LOCATION", "SERV_AREA_ID", "DISCHARGE_PROV_ID"),
  ...
  map = Map(
    ...
    "PROVIDERID" -> mapFrom("DISCHARGE_PROV_ID"),
    "PROVIDERROLE" -> literal("Discharge Provider")
  )
  ...
  afterMap = (df: DataFrame) => {
    ...
    addcolumn.filter("rn = '1' and DISCHARGE_PROV_ID is not null and DISCHARGE_PROV_ID <> '-1'")
