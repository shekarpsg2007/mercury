...
class ClinicalencounterTemptable(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  columns = List("PAT_ENC_CSN_ID","PAT_ID","HOSP_ADMSN_TIME","HOSP_DISCH_TIME","ADMIT_SOURCE_C","HSP_ACCOUNT_ID","DISCH_DISP_C","CHECKIN_TIME"
    ...
    ,"EFFECTIVE_DEPT_ID","APPT_STATUS_C","EXTERNAL_VISIT_ID","ROW_NUM", "APPT_PRC_ID","FILEID","ROW_NUM_ENC_LVL")
  ...
  val fil = df.withColumn("row_num", row_number.over(groups1))
              .withColumn("row_num_enc_lvl",row_number.over(groups))
  fil.filter("row_num = 1 and (appt_status_c is null or appt_status_c not in ('3','4','5')) and (enc_type_c is null or enc_type_c not in ('5')) " +
    "and (pat_enc_csn_id is not null and pat_enc_csn_id <> '-1') and (pat_id is not null and pat_id <> '-1')")
  }

  afterMapExceptions = Map(
    "H430416_EP2" -> ((df: DataFrame) => {
      val groups = Window.partitionBy(df("pat_enc_csn_id")).orderBy(df("update_date").desc, df("fileid").desc)
      val groups1 = Window.partitionBy(df("pat_enc_csn_id"), df("attnd_prov_id")).orderBy(df("update_date").desc, df("fileid").desc)
      val fil = df.withColumn("row_num", row_number.over(groups1))
        .withColumn("row_num_enc_lvl",row_number.over(groups))
      fil.filter("row_num = 1 and (appt_status_c is null or appt_status_c not in ('3','4','5')) and (enc_type_c is null or enc_type_c not in ('5')) " +
        "and (pat_enc_csn_id is not null and pat_enc_csn_id <> '-1') and (pat_id is not null and pat_id <> '-1') and H430_excep_col = '0'")
    })
  )
