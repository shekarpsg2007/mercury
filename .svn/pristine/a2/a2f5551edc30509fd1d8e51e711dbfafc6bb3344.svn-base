...
/**
  ...
  */
class RxorderphiCR2TempPatient(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
    ...
    map = Map(
        "ENCOUNTERID" -> literal("NULL"),
        "PATIENTDETAILQUAL" -> literal("NULL"),
        "CLIENT_DS_ID" -> mapFrom("CLIENT_DS_ID_person"),
        "FACILITYID" -> ((col: String, df: DataFrame) => {df.withColumn(col,  when(df("FACILITYID")!== "NULL", df("FACILITYID"))).withColumn("INACTIVE_FLAG", when(df("INACTIVE_FLAG")!== "NULL", df("INACTIVE_FLAG")))})
    )
}
