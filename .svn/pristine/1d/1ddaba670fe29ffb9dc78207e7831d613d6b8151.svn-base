...
class EncounterproviderReferringprovider(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  columnSelect = Map(
    "temptable" -> List("ARRIVALTIME", "DISCH_DISP_C", "PAT_ID", "HOSP_ADMSN_TIME", "HOSP_DISCH_TIME", "PAT_ENC_CSN_ID", "ATTND_PROV_ID", "UPDATE_DATE", "ENC_TYPE_C","HSP_ACCOUNT_ID",
      "ADT_PAT_CLASS_C", "ADMIT_SOURCE_C","APPT_STATUS_C", "DEPARTMENT_ID", "EFFECTIVE_DEPT_ID", "ENCOUNTER_PRIMARY_LOCATION", "SERV_AREA_ID", "REFERRAL_SOURCE_ID"),
  ...
  map = Map(
    ...
    "PROVIDERID" -> mapFrom("REFERRAL_SOURCE_ID", prefix = "ref."),
    "PROVIDERROLE" -> literal("Referring Provider")
  )
  ...
  afterMap = (df: DataFrame) => {
    ...
    val groups = Window.partitionBy(out1("PAT_ENC_CSN_ID")).orderBy(out1("UPDATE_DATE").desc, out1("FILEID").desc, out1("FILEID_ia").desc)
    val addcolumn = out1.withColumn("rn", row_number.over(groups))
    addcolumn.filter("rn = '1' and REFERRAL_SOURCE_ID is not null and REFERRAL_SOURCE_ID <> '-1'")
