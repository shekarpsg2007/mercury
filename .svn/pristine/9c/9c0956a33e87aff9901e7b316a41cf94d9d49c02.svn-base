package com.humedica.mercury.etl.hl7_v2.patient
...
/**
  ...
  */
class PatientPid(config: Map[String,String]) extends EntitySource(config: Map[String,String]) {
  ...
  map = Map(
    ...
    "MEDICALRECORDNUMBER" -> nullValue(),
    "DATEOFBIRTH" -> mapFrom("PID_F7_C1"),
    "DATEOFDEATH" -> mapFrom("PID_F29_C1")
  )

  afterMap = (df: DataFrame) => {
    val fil = df.filter("PATIENTID IS NOT NULL")
    val groups = Window.partitionBy(fil("PATIENTID")).orderBy(fil("LASTUPDATEDATE").desc)
    val addColumn = fil.withColumn("rn", row_number.over(groups))
    addColumn.filter("rn = 1").drop("rn")
  }

  mapExceptions = Map(
    ("H328218_HL7_Genesis", "DATEOFDEATH") -> ((col: String, df: DataFrame) => {
      df.withColumn(col, concat_ws("",lit("7135."),df("PID_F29_C1")))
    })
  )
