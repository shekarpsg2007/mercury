...
class PatientdetailDesceased(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  beforeJoin = Map(
    "temptable" -> ((df: DataFrame) => {
      df.filter("PAT_STATUS is not null")
    })
  )
  ...
  afterJoin = (df: DataFrame) => {
    val groups = Window.partitionBy(df("PATIENTID"), lower(df("PAT_STATUS"))).orderBy(df("UPDATE_DATE").desc)
  ...
  map = Map(
    ...
    "LOCALVALUE"-> mapFrom("PAT_STATUS", prefix=config(CLIENT_DS_ID)+".")
