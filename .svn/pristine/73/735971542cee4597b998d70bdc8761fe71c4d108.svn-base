...
class EncounterproviderAttendingprovider(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  columnSelect = Map(
    "temptable" -> List("ARRIVALTIME", "DISCH_DISP_C", "PAT_ID", "HOSP_ADMSN_TIME", "HOSP_DISCH_TIME", "PAT_ENC_CSN_ID", "ATTND_PROV_ID", "UPDATE_DATE", "ENC_TYPE_C","HSP_ACCOUNT_ID",
      "ADT_PAT_CLASS_C", "ADMIT_SOURCE_C","APPT_STATUS_C", "DEPARTMENT_ID", "EFFECTIVE_DEPT_ID", "ENCOUNTER_PRIMARY_LOCATION", "SERV_AREA_ID", "ATTND_PROV_ID"),
    "hh_pat_enc" ->List("HH_TYPE_OF_SVC_C", "HH_CONTACT_TYPE_ID", "PAT_ENC_CSN_ID"),
    "pat_enc_hsp" -> List("PAT_ENC_CSN_ID", "INP_ADM_DATE", "ED_DISPOSITION_C", "ADMIT_CONF_STAT_C", "DISCH_CONF_STAT_C", "HSP_ACCOUNT_ID", "DEPARTMENT_ID", "EXP_LEN_OF_STAY"),
  ...
  beforeJoin = Map(
    ...
    "pat_enc_hsp" -> ((df: DataFrame) => {
      val df1 = df.withColumnRenamed("DEPARTMENT_ID", "DEPARTMENT_ID_PEH").withColumnRenamed("HSP_ACCOUNT_ID", "HSP_ACCOUNT_ID_PEH")
    ...
    "medadminrec" -> ((df: DataFrame) => {
      df.dropDuplicates(Seq("MAR_ENC_CSN", "PAT_ID", "MAR_BILLING_PROV_ID")).drop("PAT_ID").repartition(1000)
    }),
    ...
    "hh_pat_enc" -> ((df: DataFrame) => {
      df.repartition(1000, df("PAT_ENC_CSN_ID"))
    })
  ...
  afterJoin = (df: DataFrame) => {
    ...
    val list_ed_disposition_c = mpvClause(table("cdr.map_predicate_values"), config(GROUP), config(CLIENT_DS_ID), "ENCOUNTERVISIT", "CLINICALENCOUNTER", "HH_PAT_ENC", "HH_TYPE_OF_SVC_C")
  ...
  map = Map(
    ...
    "PROVIDERID" -> mapFrom("ATTND_PROV_ID"),
    "PROVIDERROLE" -> literal("Attending Provider")
  )
  ...
  afterMap = (df: DataFrame) => {
    ...
    val groups = Window.partitionBy(out1("PAT_ENC_CSN_ID"), out1("ATTND_PROV_ID")).orderBy(out1("UPDATE_DATE").desc)
    val addcolumn = out1.withColumn("rn", row_number.over(groups))
    addcolumn.filter("rn = '1' and ATTND_PROV_ID is not null and ATTND_PROV_ID <> '-1'")
