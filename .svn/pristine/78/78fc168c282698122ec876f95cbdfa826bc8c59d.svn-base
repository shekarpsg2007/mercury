...
class PatientdetailRace(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  columnSelect = Map(
    "as_patdem" -> List("PATIENT_MRN", "LAST_UPDATED_DATE", "RACE_ID", "PATIENT_FIRST_NAME", "PATIENT_LAST_NAME")
  )

  beforeJoin = Map(
    "as_patdem" -> ((df: DataFrame) => {
      val groups = Window.partitionBy(df("PATIENT_MRN"), upper(df("RACE_ID"))).orderBy(df("LAST_UPDATED_DATE").desc)
      df.withColumn("rn", row_number.over(groups))
        .filter("rn=1 AND RACE_ID IS NOT NULL")
    })
  )
  ...
  map = Map(
    ...
    "LOCALVALUE" -> ((col, df) => df.withColumn(col, concat_ws(".", lit(config(CLIENT_DS_ID)), df("RACE_ID"))))
  )
