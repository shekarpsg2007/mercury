...
class ObservationShxactivity(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  beforeJoin = Map(
    ...
    "cdr.zcm_obstype_code" -> ((df: DataFrame) => {
      df.filter("groupid = '" + config(GROUP) + "' and datasrc = 'shx_act' and obstype not in ('LABRESULT', 'EXERCISE')")
  ...
  afterMap = (df: DataFrame) => {
    val groups = Window.partitionBy(df("PATIENTID"), df("LOCALCODE"), df("OBSDATE"), df("OBSTYPE"))
