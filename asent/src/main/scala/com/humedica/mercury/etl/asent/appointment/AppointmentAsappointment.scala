package com.humedica.mercury.etl.asent.appointment
...
class AppointmentAsappointment(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  afterJoin = (df: DataFrame) => {
    val fil = df.filter("APPOINTMENTSTATUSDE not in ('3', '5', '9')")
...
// Test
// val app = new AppointmentAsappointment (cfg); val ap = build(app) ; ap.show; ap.count
