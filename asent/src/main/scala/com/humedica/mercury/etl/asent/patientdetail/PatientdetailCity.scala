...
class PatientdetailCity(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  columnSelect = Map(
    "as_patdem" -> List("PATIENT_MRN", "LAST_UPDATED_DATE", "PATIENT_CITY", "PATIENT_LAST_NAME", "PATIENT_FIRST_NAME")
  )

  beforeJoin = Map(
    "as_patdem" -> ((df: DataFrame) => {
      val groups = Window.partitionBy(df("PATIENT_MRN"), upper(df("PATIENT_CITY"))).orderBy(df("LAST_UPDATED_DATE").desc)

      df.withColumn("rn", row_number.over(groups))
        .filter("rn=1 AND PATIENT_CITY IS NOT NULL")
    })
  )
  ...
  map = Map(
    ...
    "LOCALVALUE" -> mapFrom("PATIENT_CITY")
  )
