...
/**
  ...
  */
class RxorderphiEP2TempPatientId(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
    ...
    join = (dfs: Map[String, DataFrame])  => {
        ...
        var PA_ID = PA.join(ID, PA("PAT_ID_PA") === ID("PAT_ID_ID"), "left_outer")
        ...
        TEMP_PATIENTDETAIL.withColumnRenamed("HGPID", "HGPID_tpj1").withColumnRenamed("GRP_MPI", "GRP_MPI_tpj1").join(PATIENT_MPI, TEMP_PATIENT_ID_1("PATIENTID") === PATIENT_MPI("PATIENTID_MPI"), "left_outer")
