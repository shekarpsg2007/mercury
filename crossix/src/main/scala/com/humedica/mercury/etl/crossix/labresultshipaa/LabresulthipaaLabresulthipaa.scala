...
class LabresulthipaaLabresulthipaa(config: Map[String,String]) extends EntitySource(config: Map[String,String]) {

  tables = List("labresulthipaa.lab_result_hipaa", "provider")
  ...
  columnSelect = Map(
    "labresulthipaa.lab_result_hipaa" -> List("ANALYTE_SEQUENCE_NBR", "DATA_SOURCE_CODE","HI_NORMAL_VALUE_NBR","LOINC_CODE",
  ...
  join = (dfs: Map[String, DataFrame]) => {
    println("FILE ===> labresulthipaa.lab_result_hipaa")

    var provider = dfs("provider")
    var lab = dfs("labresulthipaa.lab_result_hipaa")
