...
class PatientdetailFirstname(config: Map[String,String]) extends EntitySource(config: Map[String,String]) {
  ...
  map = Map(
    ...
    "LOCALVALUE" -> cascadeFrom(Seq("PID_F5_C2", "PID_F9_C2"))
  )
  ...
  /*
    mapExceptions = Map(
      ("H285893_HL7", "LOCALVALUE") -> ((col: String, df: DataFrame) => {
        df.withColumn(col, when((df("PID_F5_C1") isNull) && (df("PID_F9_C1") isNotNull), regexpSubstr(df("PID_F9_C2"), "^[A-Za-z]+"))
            .otherwise(regexpSubstr(df("PID_F9_C2"), "^[A-Za-z]+")))
      })
      //TODO if PID_F5_C1 is null and PID_F9_C1 is not null send regexp_substr(PID_F9_C2, '^[A-Za-z]+') , else send regexp_substr(PID_F5_C2, '^[A-Za-z]+')
    )
  */
