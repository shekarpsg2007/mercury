...
class AssessmentLabbackend(config: Map[String, String]) extends EntitySource(config: Map[String, String]) {

  tables = List("lab:"+config("EMR")+"@Labresult", "cdr.unit_refrange_lab_map", "cdr.map_lab_codes", "cdr.map_unit", "cdr.v_metadata_lab",
    "cdr.map_hts_term", "cdr.unit_remap", "cdr.unit_conversion")
  ...
  beforeJoin = Map(
    "lab" -> ((df: DataFrame) => {
      df.withColumn("range_low", regexp_extract(substring_index(df("normalrange"), "-", 1), "[0-9]*[.]{0,1}[0-9]*", 0))
        ...
        .withColumnRenamed("LOCALCODE", "LOCALCODE_lab")
    }),
  ...
  join = (dfs: Map[String, DataFrame]) => {
    dfs("lab")
      .join(dfs("cdr.map_lab_codes"),
        (dfs("lab")("LOCALCODE_lab") === dfs("cdr.map_lab_codes")("MNEMONIC")) &&
          (coalesce(dfs("lab")("CLIENT_DS_ID"), lit("0")) === coalesce(dfs("cdr.map_lab_codes")("CLIENT_DS_ID_mlc"), lit("0"))),
        "left_outer")
      .join(dfs("cdr.map_unit"),
        coalesce(dfs("lab")("localunits"), dfs("lab")("localunits_inferred")) === dfs("cdr.map_unit")("LOCALCODE_mu"), "left_outer")
      .join(dfs("cdr.unit_refrange_lab_map")
        ,dfs("lab")("LOCALCODE_lab") === dfs("cdr.unit_refrange_lab_map")("LOCAL_CODE")
          &&
          (coalesce(dfs("lab")("localunits_l"), lit("NULL")) ===
            when(dfs("cdr.unit_refrange_lab_map")("local_units") === lit("ANY"), coalesce(dfs("lab")("localunits_l"), lit("NULL")))
          ...
          (coalesce(dfs("lab")("localunits_l"), lit("NULL")) ===
            when(dfs("cdr.unit_refrange_lab_map")("local_units") === lit("ANY"), coalesce(dfs("lab")("localunits_l"), lit("NULL")))
              .when(dfs("cdr.unit_refrange_lab_map")("local_units") === lit("NULL"), lit("NULL"))
              .otherwise(dfs("cdr.unit_refrange_lab_map")("local_units"))) &&
          (when(isnull(dfs("lab")("nrml_rng")) || dfs("cdr.unit_refrange_lab_map")("localrange_lower") === lit("NULL"), lit("Y"))
            .when(dfs("lab")("nrml_rng").isNotNull && isnull(dfs("lab")("range_low"))
              && (dfs("cdr.unit_refrange_lab_map")("localrange_compare") === lit("="))
              && (dfs("lab")("nrml_rng") === dfs("cdr.unit_refrange_lab_map")("localrange_lower")), lit("Y"))
            .when((dfs("cdr.unit_refrange_lab_map")("localrange_compare") === lit(">"))
              && (dfs("lab")("range_low") > dfs("cdr.unit_refrange_lab_map")("localrange_lower")) &&
...
// test
//  val   t = new assessmentLabbackend(cfg) ; val lm = build(t) ; lm.show(100)