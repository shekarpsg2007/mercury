...
class UtilDedupedPatientMedication (config: Map[String, String]) extends EntitySource(config: Map[String, String]) {
  ...
  columns=List("FILEID", "APPROVED_BY", "APPROVED_DATETIME", "CLINICAL_ENCOUNTER_ID", "CLINICAL_ORDER_TYPE",
    ...
    "DISPENSED_YN", "ADMINISTERED_EXPIRATION_DATE", "DISPENSED_EXPIRATION_DATE")
  ...
  afterJoin = (df: DataFrame) => {
    val groups = Window.partitionBy(df("PATIENT_MEDICATION_ID"), df("MEDICATION_TYPE"))
...
// test
//  val a = new UtilDedupedPatientMedication(cfg); val o = build(a); o.count
